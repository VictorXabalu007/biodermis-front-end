...
export const useRequestsData = ({
			...
			const newData = requests
				...
				.map((req) => {
					const reqProducts = req.produtos_ids
						.map((orderProduct) =>
							products.find((p) => p.id === orderProduct.id),
						)
						.map((p) => {
							if (p?.imagens) {
					...
					const notEmptyProducts = reqProducts.every((val) => val === undefined)
						? []
						: (reqProducts as Product[]);
	...
	const getTotalPayments = () => {
		...
		data.forEach((d) => {
			totalPayments += Number.parseFloat(d.valor);
			totalPaymentCount++;
		});
